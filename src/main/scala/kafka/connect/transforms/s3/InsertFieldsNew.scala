...
abstract class InsertFieldsNew[R <: ConnectRecord[R]] extends Transformation[R] {

  private final val OVERVIEW_DOC = "Add Operation & timestamp field from header of source record to target record";
  final val logger = LoggerFactory.getLogger(classOf[InsertFieldsNew[R]])
...
object InsertFieldsNew {
  //static implementation

  class Key[R <: ConnectRecord[R]] extends InsertFieldsNew[R] {
  ...
  class Value[R <: ConnectRecord[R]] extends InsertFieldsNew[R] {
